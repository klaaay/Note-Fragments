https://mp.weixin.qq.com/s/vHkxYfpuiAteMNSrpNWdsw
# 概述
## 什么是Nginx？
nginx(engine x)是一款轻量级Web服务器、反向代理服务器及电子邮件(IMAP/POP3)代理服务器。
...
> + nginx -c filename 为Nginx指定一个配置文件，代替缺省的。
> + nginx -t 不运行，而仅仅测试配置文件，nginx将检查配置文件的语法的正确性，并尝试打开配置文件中所引用到的文件。
> + nginx -v 显示nginx的版本
> + nginx -V 显示nginx的版本，编译器版本和参数配置
